...
// Get email and flow ID from query params
onMounted(() => {
...
const onSubmit = handleSubmit(async (formValues) => {
  try {
    if (!verificationFlowId.value) {
      setFieldError('code', 'Verification flow not found. Please request a new code.')
      return
    }
    ...
    if (response.success) {
      ...
      await router.push('/dashboard/settings/profile')
    } else {
...
const resendCode = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      if (response.requires2FA && response.flowId) {
        // Redirect to 2FA verification page first
        await router.push({
          path: '/auth/verify-2fa',
          query: {
            flow: response.flowId,
            return_to: `/auth/verify-email-change?email=${encodeURIComponent(email.value)}`,
          },
        })
        return
      }

      // If 2FA is not required, update flow ID
      if (response.verificationFlowId) {
        verificationFlowId.value = response.verificationFlowId
        toaster.add({
          title: 'Code Sent',
    ...
    >
      <div class="mx-auto w-full max-w-sm">
        ...
        <BaseParagraph size="sm" class="text-muted-400 mb-6">
          ...
          <span class="font-semibold text-muted-900 dark:text-muted-100">{{ email || 'your new email' }}</span>.
