...
const { isLoading, error, fetchKYBList } = usePaymentCompliance()
...
const loadKYB = async () => {
  kybList.value = await fetchKYBList(filters.value)
}
...
onMounted(async () => {
  if (import.meta.dev) {
    ...
    console.warn('[compliance/kyb.vue] onMounted - calling loadKYB()')
  }
  await loadKYB()
})
...
<template>
  <div class="space-y-6 py-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <BaseParagraph size="sm" class="mt-2 text-muted-500 dark:text-muted-400">
          View Know Your Business verification status and documents
        </BaseParagraph>
      </div>
    </div>
        ...
        <table class="w-full">
          <thead>
            <tr class="border-b border-muted-200 dark:border-muted-800">
              ...
              <th class="px-4 py-3 text-left"><BaseText size="xs" weight="semibold" class="text-muted-500 dark:text-muted-400">Registration #</BaseText></th>
          ...
          <tbody>
            ...
            <tr v-else-if="kybList.length === 0"><td colspan="7" class="px-4 py-12 text-center"><BaseText size="sm" class="text-muted-500">No KYB records found</BaseText></td></tr>
            <tr v-else v-for="kyb in kybList.slice(0, 10)" :key="kyb.id" class="border-b border-muted-100 dark:border-muted-900">
              <td class="px-4 py-3"><BaseText size="sm" weight="medium">{{ kyb.legalBusinessName }}</BaseText></td>
              <td class="px-4 py-3"><GatewayStatusBadge :status="getStatusBadge(kyb.status)" size="xs" /></td>
              <td class="px-4 py-3"><BaseText size="sm">{{ kyb.documents?.length || 0 }} documents</BaseText></td>
              <td class="px-4 py-3"><BaseText size="sm" class="font-mono">{{ kyb.registrationNumber || 'N/A' }}</BaseText></td>
              ...
              <td class="px-4 py-3"><BaseText size="sm">{{ kyb.verifiedAt ? new Date(kyb.verifiedAt).toLocaleDateString() : 'N/A' }}</BaseText></td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <BaseAlert v-if="error" color="danger" variant="pastel" class="rounded-2xl">
      <template #title>Failed to load KYB list</template>
